...
<script>
	export default {
		...
		methods:{
			export_image(){
				let save_buffer = new Promise((resolve, reject) => {//buffer二维码数据
					let filePath = `${wx.env.USER_DATA_PATH}/poster`//微信小程序的保存路径
					uni.getFileSystemManager().writeFile({//将buffer图片保存成本地临时地址
						filePath: filePath,
						data:uni.base64ToArrayBuffer(this.qrcode_data),
						encoding: 'binary',
						success: () => { 
							resolve(filePath)
						},
				...
				return new Promise((resolve, reject) => {
					save_buffer.then(filePath => {
						let ctx = uni.createCanvasContext('id_poster')
						uni.getImageInfo({
							src:this.img,
							success: (z) => {
								ctx.drawImage(z.path, 0, 0, this.width, this.height)
